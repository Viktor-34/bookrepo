# 📋 План разработки: Social Posts Saver на базе Hoarder

## 🎯 Цель проекта
Создать персональный сервис для сохранения постов из X.com и Facebook с:
- Превью изображений (включая множественные)
- Тегированием
- Браузерным расширением
- Импортом CSV
- Простым деплоем на VPS
...
1. **CSV Import** - импорт списка ссылок
2. **Улучшенный Grid View** - оптимизированное отображение множественных изображений
3. **Social Media Optimization** - специальная обработка постов из Twitter/Facebook
4. **Локальная версия** - упрощенный setup без внешних зависимостей (опционально)
...
**Задачи:**
1. ✅ Форкнуть репозиторий hoarder-app/hoarder на GitHub
2. ✅ Клонировать форк в `bookrepo/`
3. ✅ Изучить структуру проекта:
   - `/apps/web` - Next.js frontend
   - `/apps/workers` - Background workers
   - `/apps/mobile` - React Native apps
   - `/packages` - Shared code
4. ✅ Запустить локально через Docker Compose
5. ✅ Протестировать базовый функционал
6. ✅ Изучить браузерное расширение (`/apps/browser-extension`)

**Deliverables:**
- Работающий локальный инстанс Hoarder
- Понимание архитектуры
- Список файлов для модификации

---

### **Этап 2: CSV Import (День 2-3)**
...
### **Этап 3: Улучшенный Grid View для изображений (День 4-5)**
...
| Этап | Задача | Время |
|------|--------|-------|
| 1 | Setup & Исследование | 1 день |
| 2 | CSV Import | 2-3 дня |
| 3 | Grid View | 2 дня |
| 4 | Social Media Optimization | 2 дня |
| 5 | VPS Deployment | 2 дня |
| 6 | Тестирование | 1 день |
| 7 | Локальная версия (опц.) | 2 дня |
| **ИТОГО** | **MVP** | **10-12 дней** |

---

## 🎯 Минимальный MVP (5 дней быстрого старта)

Если нужно очень быстро:

**День 1:**
- Форк + локальный setup
...
**День 4:**
- VPS Deployment (с дефолтным UI Hoarder)

**День 5:**
- Тестирование + документация

**Результат:** Работающий сервис с расширением (уже есть) + CSV импорт + деплой на VPS

Улучшение Grid View и Social Media оптимизацию можно добавить позже итеративно.
...
2. ⏳ **Форкнуть репозиторий** - Ждем подтверждения от пользователя
3. ⏳ **Клонировать и запустить локально**
4. ⏳ **Начать разработку CSV Import**

---

## ❓ Вопросы для уточнения

1. **VPS провайдер:** Есть предпочтения? (DigitalOcean, Hetzner, Vultr, etc.)
2. **Домен:** Уже есть домен для хостинга?
3. **AI функции:** Нужны ли AI-теги или можно отключить для экономии?
4. **Бюджет VPS:** ~$10-20/месяц (4GB RAM, 2 vCPU) достаточно?
5. **Приоритет:** Начать с MVP за 5 дней или полная версия за 10 дней?

---

## 📝 Заметки

- Hoarder активно развивается (последний релиз март 2025)
